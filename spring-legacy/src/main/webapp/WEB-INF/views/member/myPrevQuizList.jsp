			...
			<div class="container">
				...
				<ul class="pagination center">
					...
					<c:if test="${ pageMaker.prev eq true }">
						<li><a
							href="/member/myPrevQuizList?pageNum=${ pageMaker.startPage - 1 }&type=${ pageMaker.cri.type }&keyword=${ pageMaker.cri.keyword }#prevQuiz">
						</a></li>
					</c:if>
					...
					<c:forEach var="i" begin="${ pageMaker.startPage }"
						...
						<li
							...
							href="/member/myPrevQuizList?pageNum=${ i }&type=${ pageMaker.cri.type }&keyword=${ pageMaker.cri.keyword }#prevQuiz">${ i }</a></li>
					</c:forEach>

					<%-- 다음 --%>
					<c:if test="${ pageMaker.next eq true }">
						<li><a
							href="/member/myPrevQuizList?pageNum=${ pageMaker.endPage + 1 }&type=${ pageMaker.cri.type }&keyword=${ pageMaker.cri.keyword }#prevQuiz">
						</a></li>
					</c:if>
				</ul>
